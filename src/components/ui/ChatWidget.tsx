...
const ChatMessage: React.FC<ChatMessageProps> = ({ message }) => {
    ...
    return (
        <li className={`flex items-end gap-2 ${isUser ? 'justify-end' : 'justify-start'}`}>
        ...
        <div className="fixed bottom-6 right-6 z-[80] font-sans">
            ...
            <div 
                ...
                <header className="flex justify-between items-center p-4 bg-cream border-b rounded-t-lg">
                    <h3 id="chat-heading" className="font-bold text-brown-gray">Чат с поддержкой</h3>
                    <button onClick={toggleChat} className="text-brown-gray hover:opacity-75" aria-label="Закрыть чат">
                        <CloseIcon />
                    </button>
                </header>
                <ul aria-live="polite" className="flex-1 p-4 overflow-y-auto space-y-4">
                    {messages.map((msg: Message) => <ChatMessage key={msg.id} message={msg} />)}
                </ul>
